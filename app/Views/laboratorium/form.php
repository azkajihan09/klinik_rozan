...
<div class="content-header px-0 pt-0 pb-2">
    <div class="container-fluid px-0 d-flex flex-wrap justify-content-between align-items-center gap-3">
        <div>
            <h1 class="mb-1">Order Pemeriksaan Laboratorium</h1>
            <ol class="breadcrumb mb-0 small">
                <li class="breadcrumb-item"><a href="<?= site_url('dashboard') ?>">Dashboard</a></li>
                <li class="breadcrumb-item"><a href="<?= site_url('module/laboratorium') ?>">Laboratorium</a></li>
                <li class="breadcrumb-item active" aria-current="page">Order Baru</li>
            </ol>
        </div>
        <a href="<?= site_url('module/laboratorium') ?>" class="btn btn-outline-secondary">Kembali</a>
    </div>
</div>
...
<form method="post" action="<?= site_url('module/laboratorium') ?>">
        ...
        <div class="card-body p-0">
            <?php if (empty($antrian)): ?>
                <div class="app-empty-state">
                    <p class="mb-2">Belum ada pasien terdaftar hari ini.</p><a href="<?= site_url('module/registrasi/new') ?>" class="btn btn-primary btn-sm">Registrasi Pasien</a>
                </div>
            <?php else: ?>
                <div class="list-group list-group-flush app-list-stack">
                    <?php foreach ($antrian as $i => $a): ?>
                        <label class="list-group-item app-queue-item d-flex align-items-center gap-3">
                            ...
                            <span class="app-avatar bg-info"><?= str_pad((string)($i + 1), 2, '0', STR_PAD_LEFT) ?></span>
    ...
    <div class="card card-outline card-info animate-fade-up">
        <div class="card-header">
            <h3 class="card-title">Data Permintaan Lab</h3><span class="badge text-bg-info">No: <?= esc($noOrder) ?></span>
        </div>
        <div class="card-body">
            <div class="row g-3">
    ...
    <div class="card card-outline card-secondary animate-fade-up">
        ...
        <div class="card-body">
            <div class="row g-3">
                <div class="col-lg-6"><label class="form-label">Diagnosa Klinis <span class="text-danger">*</span></label><textarea name="diagnosa_klinis" class="form-control" rows="3" placeholder="Diagnosa klinis / indikasi pemeriksaan lab..." required maxlength="80"></textarea>
                    <div class="form-text">Maksimal 80 karakter.</div>
                </div>
                <div class="col-lg-6"><label class="form-label">Informasi Tambahan</label><textarea name="informasi_tambahan" class="form-control" rows="3" placeholder="Catatan: puasa, riwayat obat, dll..." maxlength="60"></textarea>
    ...
    <div class="card card-outline card-primary animate-fade-up">
        <div class="card-body d-flex flex-wrap justify-content-between align-items-center gap-3">
            <div>
                <h3 class="card-title mb-1">Kirim Permintaan Lab</h3>
                <p class="text-muted mb-0 small">Order akan dikirim ke unit laboratorium untuk diproses.</p>
            </div>
            <div class="d-flex flex-wrap gap-2">
                <button type="submit" class="btn btn-primary">Kirim Order Lab</button>
                <a href="<?= site_url('module/laboratorium') ?>" class="btn btn-outline-secondary">Batal</a>
            </div>
        </div>
    </div>
