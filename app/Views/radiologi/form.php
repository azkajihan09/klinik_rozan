...
<div class="page-header">
    <div>
        <h1>📡 Order Pemeriksaan Radiologi</h1>
        <p class="breadcrumb">
    ...
    <a href="<?= site_url('module/radiologi') ?>" class="btn btn-secondary">
        ...
        <div style="display:grid;gap:6px;max-height:200px;overflow-y:auto;padding:4px">
            <?php foreach($antrian as $i => $a): ?>
                <label style="display:flex;align-items:center;gap:12px;padding:10px 14px;border:2px solid var(--border,#e2e8f0);border-radius:10px;cursor:pointer;transition:all .2s" class="pasien-radio">
                    <input type="radio" name="no_rawat" value="<?= esc($a['no_rawat']) ?>" style="width:16px;height:16px;accent-color:#f59e0b" required <?= $i === 0 ? 'checked' : '' ?>>
                    <div style="width:32px;height:32px;border-radius:8px;background:#fef3c7;display:flex;align-items:center;justify-content:center;font-weight:700;font-size:11px;color:#92400e">
...
<div class="card animate-fade-up" style="animation-delay:.1s">
    <div class="card-header">
        <h3>
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="2" y="2" width="20" height="20" rx="2"/><circle cx="12" cy="12" r="4"/><path d="M12 2v4M12 18v4M2 12h4M18 12h4"/></svg>
            Data Permintaan Radiologi
        </h3>
        <span class="badge badge-warning">No: <?= esc($noOrder) ?></span>
    </div>
    <div class="form-grid">
        <div class="form-group">
            <label class="form-label">No. Order</label>
            <input type="text" name="noorder" value="<?= esc($noOrder) ?>" class="form-control" style="background:#f1f5f9;font-weight:700;color:#d97706" readonly>
...
<div class="card animate-fade-up" style="animation-delay:.2s">
    ...
    <div class="form-grid">
        <div class="form-group">
            ...
            <textarea name="diagnosa_klinis" class="form-control" rows="3" placeholder="Diagnosa klinis / indikasi pemeriksaan radiologi..." required></textarea>
        </div>
        <div class="form-group">
            <label class="form-label">Informasi Tambahan</label>
            <textarea name="informasi_tambahan" class="form-control" rows="3" placeholder="Jenis pemeriksaan (Rontgen, USG, CT-Scan, dll), catatan khusus..."></textarea>
        </div>
    </div>
</div>
...
<!-- SUBMIT -->
<div class="card animate-fade-up" style="animation-delay:.3s;background:linear-gradient(135deg,#fffbeb,#fef3c7);border-color:#fcd34d">
    <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:12px">
        <div>
            <p style="font-weight:600;font-size:14px;color:#92400e">📡 Kirim Permintaan Radiologi</p>
            <p style="font-size:12px;color:var(--text-muted,#64748b)">Order akan dikirim ke unit radiologi untuk dijadwalkan.</p>
        </div>
        <div style="display:flex;gap:10px">
            <button type="submit" class="btn btn-primary" style="background:linear-gradient(135deg,#d97706,#f59e0b)">
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="22" y1="2" x2="11" y2="13"/><polygon points="22 2 15 22 11 13 2 9 22 2"/></svg>
                Kirim Order Radiologi
            </button>
            <a href="<?= site_url('module/radiologi') ?>" class="btn btn-secondary">Batal</a>
        </div>
    </div>
</div>
...
.pasien-radio:hover { border-color: #fcd34d; background: #fefce8; }
</style>
